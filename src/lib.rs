...
//! * Also, when polling for `StatType::Thread` the memory usage will be 0. See Spork struct documentation for details.
//!
//! ## OSX
//! * When polling for `StatType::Thread`, memory usage reported will be different than Linux, See Spork struct documentation for details.
//!
//! ## Linux
//! * When polling for `StatType::Thread`, memory usage will be the same as the overall process memory usage.
//!
//! ## Basic Usage
//!
//! ```
...
/// The calling system platform, such as `Linux`, `Windows`, etc.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Platform {
  Linux,
  MacOS,
...
/// multiple threads do not interfere with each other's clocks. Using `stat` and `stat_with_cpus` with
...
///
/// ## Windows
/// As noted above, the memory stat for windows returns 0 on `StatType::Thread`. Since threads share the same address space reading per-thread memory
/// usage doesn't really make sense anyways, so this should be ignored regardless.
///
/// TLDR: Be careful using `stat` and `stat_with_cpus` with `StatType::Process` across multiple threads running concurrently, and don't bother
/// using the memory usage values with `StatType::Thread` on any platform.
///
#[derive(Clone, Debug)]
pub struct Spork {
  history: History,
...
impl Spork {
  ...
  /// Clear the stats history for the process or calling thread.
  ///
  /// This library works by tracking the timestamp of the last stats poll, per thread, such that polls from different threads do not interfere with each other.
